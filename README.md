...
MySQL의 파티셔닝 기능을 적극 활용하여 **연도별·기업별 재무 분석의 성능을 극대화**하고, SQL 기반 지표 계산을 통해 다양한 <br> **재무 위험 신호 및 성장 패턴을 정량적으로 추출**합니다.
...
- 📥 대용량 CSV 재무 데이터 MySQL에 정제·적재
- 🔀 연도·기업 기준 파티셔닝 처리로 분석 성능 최적화
- 📈 주요 재무 지표 산출: **안정성, 수익성, 성장성**
- 🔍 이상치 탐지 및 **위험 기업 자동 식별**
- 📊 산업군·기업 간 재무 건전성 비교 분석
...
&emsp;&emsp;&emsp;&emsp;&emsp;부채비율, 영업이익, ROE, ROA 등
...
다단계 파티셔닝을 통해 **연도별 + 기업 분산 처리 구조**를 구축합니다:
...
이자보상배율: 업계 평균 대비 급락 → 위험 신호 감지

▶ 우리금융지주
순이익: 전년 대비 -25% 감소

ROE 하락 + 비용 증가 → 건전성 저하 경고
